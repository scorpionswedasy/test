...
class ZegoLiveAudioRoomManager {
  ...
  // تم تعديل هذا الجزء
  ValueNotifier<int> get seatCountNotifier =>
      roomSeatService?.seatCountNotifier ?? ValueNotifier<int>(0);
  ...
  void updateSeatCount(int newSeatCount) {
    roomSeatService?.updateSeatCount(newSeatCount);
  }
  ...
  void onRoomCommandReceived(OnRoomCommandReceivedEvent event) {
    try {
      ...
      if (messageMap.keys.contains('room_command_type')) {
        ...
        if (type == 'seat_count_changed') {
          final newCount = messageMap['new_seat_count'];
          roomSeatService?.updateSeatCount(newCount);
          return;
        }
        ...
        if (receiverID == ZEGOSDKManager().currentUser!.userID) {
          if (type == RoomCommandType.muteSpeaker) {
            ...
            ScaffoldMessenger.of(navigatorKey.currentContext!)
                .showSnackBar(const SnackBar(content: Text('You have been kicked out of the room by the host')));
            logoutRoom();
      ...
      debugPrint('Error processing room command: $e');
    }
  }
